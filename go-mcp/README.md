...
[MCP](https://modelcontextprotocol.io) server exposing the UvIndex SDK as
two agent tools — `uv-index_list` and `uv-index_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/uv-index-mcp)
make build

# 2. Provide credentials via the environment
export UV_INDEX_APIKEY=sk_live_xxx
...
```jsonc
// uv-index_list: first page of records
{ "entity": "uv_index" }
{ "entity": "uv_index", "query": { } }

// uv-index_load: one record by id
{ "entity": "uv_index", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/uv-index-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user uv-index \
     -- "$PWD"/dist/*/uv-index-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `uv-index_list` and `uv-index_load` tools now appear
   in new sessions. Ask the agent to *"list uv_index using uv-index"*
   and it calls `uv-index_list` with `{"entity":"uv_index"}`.
...
```sh
export UV_INDEX_APIKEY=sk_live_xxx            # API key
export UV_INDEX_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `uv-index_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `uv-index_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `UV_INDEX_APIKEY` | API key sent with every request. |
| `UV_INDEX_BASE` | Optional override of the API base URL. |
