...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.UVINDEX_TEST_LIVE ||
    'TRUE' === process.env.UVINDEX_TEST_OVERRIDE
  ) {
  ...
  m.UVINDEX_TEST_EXPLAIN = process.env.UVINDEX_TEST_EXPLAIN || m.UVINDEX_TEST_EXPLAIN
