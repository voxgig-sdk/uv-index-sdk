...
//     (\`new UvIndexSDK(...)\` / \`UvIndexSDK.test(...)\` become
//     \`UvIndexSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"uv_index":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UvIndexSDK(...)\` and any
// \`UvIndexSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UvIndexSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UvIndexSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
