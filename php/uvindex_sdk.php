...
class UvIndexSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = UvIndexHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = UvIndexHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!UvIndexFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, UvIndexFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return UvIndexUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = UvIndexHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = UvIndexHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = UvIndexHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new UvIndexError($op . "_allow",
                "UvIndexSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = UvIndexHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new UvIndexError("graphql_error",
                "UvIndexSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function UvIndex($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_uv_index === null) {
                $this->_uv_index = new UvIndexEntity($this, null);
            }
            return $this->_uv_index;
        }
        return new UvIndexEntity($this, $data);
    }
