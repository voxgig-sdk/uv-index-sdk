...
{
    private static function utility(): UvIndexUtility
    {
        return new UvIndexUtility();
    }
    ...
    private static function ctx(array $over = []): UvIndexContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new UvIndexContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new UvIndexOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof UvIndexError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new UvIndexOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = UvIndexMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new UvIndexOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = UvIndexMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new UvIndexOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = UvIndexMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UvIndexSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = UvIndexMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new UvIndexResponse([]),
            'result' => new UvIndexResult([]),
        ]);
        [, $err] = UvIndexMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new UvIndexSpec([]),
            'response' => null,
            'result' => new UvIndexResult([]),
        ]);
        [, $err] = UvIndexMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new UvIndexSpec(['step' => 's']),
            'response' => new UvIndexResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new UvIndexResult([]),
        ]);
        [, $err] = UvIndexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new UvIndexSpec(['step' => 's']),
            'response' => new UvIndexResponse(self::resp(200, ['v' => 1])),
            'result' => new UvIndexResult([]),
        ]);
        [, $err] = UvIndexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new UvIndexSpec(['step' => 's']),
            'response' => new UvIndexResponse(self::resp(200, ['v' => 2])),
            'result' => new UvIndexResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        UvIndexMakeResponse::call($ctx);
    ...
    {
        $preset = new UvIndexResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new UvIndexSpec([]),
            'response' => new UvIndexResponse([]),
            'result' => new UvIndexResult([]),
        ...
        [$got, $err] = UvIndexMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new UvIndexResult([])]);
        [, $err] = UvIndexMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new UvIndexSpec([]), 'result' => null]);
        [, $err] = UvIndexMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UvIndexSpec(['step' => 's']),
            'result' => new UvIndexResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new UvIndexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UvIndexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new UvIndexSpec(['step' => 's']),
            'result' => new UvIndexResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new UvIndexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = UvIndexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new UvIndexResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new UvIndexSpec([]),
            'result' => new UvIndexResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = UvIndexMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UvIndexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new UvIndexError('boom', 'boom');
        $utility->fetcher = function (UvIndexContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UvIndexSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = UvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (UvIndexContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = UvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(UvIndexResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (UvIndexContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new UvIndexSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = UvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new UvIndexResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new UvIndexSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = UvIndexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = UvIndexMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = UvIndexMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new UvIndexResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, UvIndexDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new UvIndexResult(['ok' => false])]);
        $this->expectException(UvIndexError::class);
        UvIndexDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new UvIndexResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', UvIndexMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        UvIndexMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new UvIndexBaseFeature();
        $b = new UvIndexBaseFeature();
        UvIndexFeatureAdd::call($ctx, $a);
        UvIndexFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): UvIndexBaseFeature
    {
        $f = new UvIndexBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        UvIndexFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): UvIndexContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new UvIndexSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = UvIndexPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new UvIndexResponse(['headers' => null]),
            'result' => new UvIndexResult([]),
        ]);
        UvIndexResultHeaders::call($ctx);
