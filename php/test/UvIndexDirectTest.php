...
class UvIndexDirectTest extends TestCase
{
    public function test_direct_load_uv_index(): void
    {
        $setup = uv_index_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-uv_index", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function uv_index_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new UvIndexSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
