...
class FtCtrl extends UvIndexControl
{
...
class FtClient
{
    ...
    private UvIndexUtility $utility;

    public function __construct(UvIndexUtility $utility, array $options)
    {
    ...
    public function get_utility(): UvIndexUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UvIndexContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new UvIndexUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (UvIndexContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UvIndexConfig::make_feature($name);
    ...
    private static function buildUrl(UvIndexSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UvIndexError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UvIndexResponse($fetched);
        }
    ...
    private function fail(UvIndexContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UvIndexContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
