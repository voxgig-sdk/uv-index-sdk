...
local UvIndexEntity = {}
UvIndexEntity.__index = UvIndexEntity


function UvIndexEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, UvIndexEntity)
  self._name = "uv_index"
  self._client = client
...
function UvIndexEntity:get_name()
  return self._name
end


function UvIndexEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return UvIndexEntity.new(self._client, opts)
end
...
function UvIndexEntity:mark_deleted()
  self._deleted = true
end


function UvIndexEntity:deleted()
  return true == self._deleted
end


function UvIndexEntity:data_set(args)
  if args ~= nil then
...
function UvIndexEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function UvIndexEntity:match_set(args)
  if args ~= nil then
...
function UvIndexEntity:match_get()
...
function UvIndexEntity:stream(action, args, callopts)
...
---@param reqmatch UvIndexLoadMatch
---@param ctrl? table
---@return UvIndex
---@return string? err
function UvIndexEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function UvIndexEntity:_run_op(ctx, post_done)
  local utility = self._utility
