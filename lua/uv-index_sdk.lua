...
UvIndexSDK._make_feature = _make_feature


function UvIndexSDK.new(options)
  local self = setmetatable({}, UvIndexSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function UvIndexSDK:options_map()
...
function UvIndexSDK:get_utility()
  return Utility.copy(self._utility)
end


function UvIndexSDK:get_root_ctx()
  return self._rootctx
end


function UvIndexSDK:prepare(fetchargs)
...
function UvIndexSDK:direct(fetchargs)
...
-- by passing it.
function UvIndexSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function UvIndexSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "UvIndexSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function UvIndexSDK:UvIndex(data)
  ...
  if data == nil then
    if self._uv_index == nil then
      self._uv_index = EntityMod.new(self, nil)
    end
...
function UvIndexSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = UvIndexSDK.new(sdkopts)
  sdk.mode = "test"
