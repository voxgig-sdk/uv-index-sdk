...
boru-driven command-line client **and** interactive REPL for the UvIndex
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/uv-index-cli)
make build
...
./uv-index-cli load 1 uv_index            # {id:1} shorthand
./uv-index-cli load '{id:1}' uv_index       # explicit match map

# 5. Override the API base URL for a single call
UV_INDEX_BASE=https://api.example.com ./uv-index-cli load 1 uv_index

# 6. No arguments -> interactive REPL
./uv-index-cli
uv-index> load 1 uv_index
uv-index> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/uv-index-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/uv-index-cli load 1 uv_index
   ```
...
```sh
./uv-index-cli load 1 uv_index          # scalar shorthand for {id:1}
./uv-index-cli load '{id:1}' uv_index     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `uv-index>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `uv_index`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/uv-index-cli`. |
...
(`./uv-index-cli <expr>`) and interactively (the REPL), and expressions compose the
