...
import (
	...
	omni "github.com/voxgig-sdk/uv-index-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UvIndexError); is {
				out := map[string]any{
					"name":    "UvIndexError",
...
func omniCtx(arg any, client *sdk.UvIndexSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UvIndexSDK); is {
			client = live
