...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = UvIndexOmni.make_runner(TEST_JSON_FILE, UvIndexSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : UvIndexOmni.errify(err)
    end
