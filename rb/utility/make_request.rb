...
module UvIndexUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = UvIndexResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UvIndexResponse.new(fetched)
    else
