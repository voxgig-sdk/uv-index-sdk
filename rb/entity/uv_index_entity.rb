...
class UvIndexEntity
  ...
  def make
    opts = @_entopts.dup
    UvIndexEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UvIndexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [UvIndex, Hash] the current UvIndex data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UvIndexHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of UvIndex fields)
  def match_get
  ...
  #
  # @param reqmatch [UvIndexLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.UvIndex.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [UvIndex, Hash] the loaded UvIndex; raises UvIndexError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UvIndexHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
